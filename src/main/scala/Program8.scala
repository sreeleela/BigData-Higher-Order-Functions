object Program8 {
  def main(args: Array[String]): Unit = {
    randomNumbersROfThousandSamples()
  }
  def  randomNumbersROfThousandSamples(): Unit = {
  ...
  def createAllSamples(sampleMeans : Array[BigDecimal],samplesCount : Int): Unit ={
    ...
    var sampleSeq = 1 to 1000
    var sampleArray = sampleSeq.toArray
    var count = -1
    var sampleArrayFinal = new Array[Int](1000)
    var sampleArrayTemp = sampleArrayFinal.map(x => if(count < 1000){count = count + 1; sampleArrayFinal.update(count,sampleArrayIndex(count))})
  ...
  def createSample(): Array[Int] ={
    var sampleArrayIndex = sampleNoRepeats(1 to 50000,1000).toArray[Int]
    sampleArrayIndex
  }
